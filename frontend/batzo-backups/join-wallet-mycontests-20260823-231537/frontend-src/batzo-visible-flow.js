...
(function(){
  ...
  function contestFlow(){
    try{
      if(typeof window.openBatzoContest === "function"){
        const list = window.BATZO_CONTESTS || [];
        const contest = list[0] || {
          name:"Mega Contest",
          prize:"₹50 Lakhs",
          entry:"₹49",
          spots:"2.1L",
        ...
        window.openBatzoContest(contest);
        return true;
      }

      window.dispatchEvent(new CustomEvent("batzo:contest",{
        detail:{
          contest:{
            name:"Mega Contest",
            prize:"₹50 Lakhs",
      ...
      console.error("BATZO CONTEST ROUTE ERROR",err);
      return false;
    }
  }
  ...
  function teamFlow(){
    try{
      ...
      console.error("BATZO TEAM ROUTE ERROR",err);
      return false;
    }
  }

  /*
   * WINDOW CAPTURE runs before document-level duplicate handlers.
   * This makes the Upcoming VIEW CONTESTS button reliable.
   */
  if(!window.__BATZO_FINAL_MASTER_ROUTER__){
    window.__BATZO_FINAL_MASTER_ROUTER__ = true;
  ...
  /*
   * Also repair dynamically-rendered Upcoming buttons.
   */
  const observer=new MutationObserver(function(){
    document.querySelectorAll(
      'button,a,.bz-contest-action,[class*="contest"]'
    ).forEach(function(el){
      const text=(el.innerText || el.textContent || "")
        .replace(/\s+/g," ")
        .trim()
      ...
      if(text.includes("VIEW CONTESTS")){
        el.setAttribute("data-batzo-contest-route","1");
      }
    });
  });
