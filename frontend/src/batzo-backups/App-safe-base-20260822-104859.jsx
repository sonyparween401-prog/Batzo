import BatzoDream11Flow from "./BatzoDream11Flow.jsx";
...
function BatzoDream11Launcher(){
 const [open,setOpen]=React.useState(false);
 React.useEffect(()=>{
  const fn=e=>{
   const t=(e.target?.innerText||"").toUpperCase();
   if(t.includes("CONTEST") || t.includes("CREATE MY TEAM") || t.includes("JOIN CONTEST")){
    setOpen(true);
   }
  };
  (()=>{})("click",fn,true);
...
if(typeof window!=="undefined" && !window.__BATZO_DREAM11_HOOKED){
 window.__BATZO_DREAM11_HOOKED=true;
 const mount=document.createElement("div");
 mount.id="batzo-dream11-root";
 document.addEventListener("DOMContentLoaded",()=>document.body.appendChild(mount),{once:true});
}
