...
/* BATZO_MASTER_CREATE_JOIN_V3 */
(function(){
  if(typeof window==="undefined") return;
  if(window.__BATZO_MASTER_CREATE_JOIN_V3__) return;
  window.__BATZO_MASTER_CREATE_JOIN_V3__=true;

  const KEY="batzo_master_join_pending_v3";
  ...
  function isTeamSaveButton(el){
    const t=text(el);
    return (
      t.includes("confirm team") ||
      t.includes("save team") ||
      t.includes("continue") && (
        t.includes("11/11") ||
        t.includes("team")
      )
  ...
  function isTeamConfirmed(){
    const body=text(document.body);
    return (
      body.includes("team confirmed") ||
      body.includes("team saved") ||
      body.includes("my teams")
    );
  }
  ...
  /*
    IMPORTANT:
    We do NOT manufacture a new Join button.
    We let the existing React contest screen render normally.
    After the team is successfully confirmed, automatically return
    to the contest screen exactly as the user's manual Back action did.
  */
  document.addEventListener("click",function(ev){
    const el=ev.target?.closest?.("button,[role='button']");
    if(!el) return;

    if(isTeamSaveButton(el)){
      markPending();

      /*
        Give React enough time to finish saving/rendering.
        Then, if the team-confirmed screen is reached, automatically
        perform the same navigation the user currently performs manually.
      */
      setTimeout(function(){
        let tries=0;
        ...
        const timer=setInterval(function(){
          ...
          if(isTeamConfirmed()){
            ...
            setTimeout(function(){
              ...
              /*
                If the current component has no Back button,
                use browser history as the same fallback.
              */
  ...
  /*
    Safety: if app was reopened after a completed team save,
    remove stale pending flag instead of looping.
  */
  window.addEventListener("load",function(){
    setTimeout(function(){
      if(!pending()) return;
      if(!isTeamConfirmed()) clearPending();
    },1000);
  });
})();
